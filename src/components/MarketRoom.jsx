...
import { TrendingUp, MapPin, Award, ArrowUpRight, ArrowDownRight, RefreshCw } from 'lucide-react';
// थेट Firebase / Backend सर्व्हिस इम्पोर्ट केली आहे
...
const MarketRoom = () => {
  ...
  // ── १. थेट Firebase/Backend मधून सर्व शेतकऱ्यांचा डेटा आणणे ──
  const fetchRealMarketData = async () => {
    setIsLoading(true);
    try {
      // Firebase मधून 'उत्पन्न' (income) असलेल्या सर्व शेतकऱ्यांच्या पावत्या आणा
      const allSales = await dbService.getDocumentsByField('ledger', 'type', 'income');
      
      const data = {};
      allSales.forEach(sale => {
        if (!sale.crop && !sale.commodity) return; // पीक नसेल तर सोडून द्या
        ...
        // विक्रीची रक्कम नोंदवा (इतर शेतकऱ्यांनी दिलेली)
        data[cropName][marketName].prices.push(Number(sale.amount));
      });
      ...
      // जर सर्व्हरवर खऱ्या नोंदी नसतील, तर नमुना माहिती (Mock Data) दाखवा
      ...
      console.error("बाजारभाव आणताना चूक झाली:", error);
      useMockData(); // इंटरनेट किंवा API बंद असल्यास अ‍ॅप थांबणार नाही
    } finally {
      setIsLoading(false);
    }
  };

  // जेव्हा खोली उघडेल तेव्हा सर्व्हरवरून डेटा आणा
  useEffect(() => {
    fetchRealMarketData();
  }, []);

  // नमुना डेटा (जर Firebase मध्ये इतर शेतकऱ्यांचा डेटा नसेल तर)
  const useMockData = () => {
    setMarketData({
      'कांदा': [
    ...
    <div className="flex flex-col h-full bg-[#fdf8f0] p-4 space-y-6 overflow-y-auto pb-32">
      ...
      <div className="pt-2 flex justify-between items-start">
        <div>
          ...
          <p className="text-sm font-bold text-[#8b5e3c] mt-1 tracking-wide">सर्व्हरवरील ताजी माहिती (Live)</p>
        </div>
        <button onClick={fetchRealMarketData} disabled={isLoading} className="p-2 bg-white rounded-full border border-[#d4a853]/30 text-[#8b5e3c] hover:bg-[#d4edda] transition-colors">
      ...
      {isLoading ? (
        <div className="flex justify-center py-4"><span className="animate-spin text-2xl">⏳</span></div>
      ) : crops.length > 0 ? (
      ...
      {!isLoading && currentMarkets.length > 1 && (
